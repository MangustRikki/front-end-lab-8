...
//   Your code goes here
  ...
  function fighter(attackFighter) {
    const combatHistory = {
        wins: 0,
        loses: 0
    };
    ...
    function fight(defFighter) {
        let block = Math.random() >= 0.5;
        
        if(!block) {
            defFighter.getStats().hp -= attackFighter.attack;
            if (defFighter.getStats().hp <= 0) {
                defFighter.getCombatHistory().loses++;
                combatHistory.wins++;
            }
            return true;
        }
